...
public class CarControllerP2 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            speedInput = Input.GetAxis("VerticalPlayer2") * forwardAccel * 1000f;
        }
        else if (Input.GetAxis("VerticalPlayer2") < 0) 
        {
            speedInput = Input.GetAxis("VerticalPlayer2") * reverseAccel * 1000f;
        }
        ...
        if (grounded)
        {
            transform.rotation  = Quaternion.Euler(transform.rotation.eulerAngles + new Vector3(0f, turnInput * turnStrength * Time.deltaTime * Input.GetAxis("VerticalPlayer2"), 0f));
        }
